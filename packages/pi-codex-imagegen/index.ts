	...
	try {
		...
		return registerCodeModeExtensionTools(pi, () => [
			adaptToolForCodeMode(tool, {
				usage:
					"generatedImage(await tools.image_gen__imagegen({prompt:string,referenced_image_paths?:string[],num_last_images_to_include?:number})) // prompt: include target aspect ratio and quality; no selectors=generate; paths/recent count=edit; never text/serialize base64 result",
