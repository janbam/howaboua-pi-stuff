...
- Keep `Cache` on Pi's single official status row beside `Codex`; never replace the footer or use a widget. Pi owns cache percentages, diagnostics status shows only HIT/MISS and transport/continuation state.
